...
** replicating patterns from syncer to mcp
 - [X] Initializing the storage backend using command-line options.

 - [ ] health and readiness probes for mcp via implementing the required interface for the controller runtime

 - [ ] Passing the initialized storage instance to the MCPStorageServer.
 - [ ] Ensuring the actual MCP server start call is blocking and serves as the main        execution loop.
 - [ ] Ensuring the MCP server respects the context for graceful shutdown.
